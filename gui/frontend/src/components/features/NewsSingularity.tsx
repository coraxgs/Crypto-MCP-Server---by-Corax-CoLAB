...
const Singularity = () => {
  ...
  const fragmentShader = `
    ...
    varying vec3 vNormal;
    void main() {
      vec3 color = vec3(0.1, 0.4, 0.8); // Base blue
...
export default function NewsSingularity() {
  ...
  useEffect(() => {
    // Mock news stream
    const topics = ['SEC Approval', 'Whale Moves', 'Network Upgrade', 'Exchange Hack', 'New Regulation'];
    const sentiments = ['positive', 'negative', 'neutral'];

    const interval = setInterval(() => {
      const newNodes = Array.from({ length: 5 }).map(() => ({
        id: Math.random(),
