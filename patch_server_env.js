...
if (!code.includes('const MCP_LLM')) {
  code = code.replace("const MCP_CCXT = process.env.MCP_CCXT || 'http://127.0.0.1:7001/mcp';", "");
  code = code.replace("const MCP_PORTFOLIO = process.env.MCP_PORTFOLIO || 'http://127.0.0.1:7004/mcp';", "");
  code = code.replace("const PORT = process.env.PORT || 4000;", newCode + "\nconst PORT = process.env.PORT || 4000;");

  // Fix generic endpoint
  code = code.replace("const mcpUrl = process.env[mcp];", "const mcpUrl = mcpUrls[mcp];");
